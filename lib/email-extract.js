...
- "AD1" / "AD1 Global" / "AD1 Hospitality" / "Alex Fridzon" (alex.fridzon@ad1hospitality.com) / "Gisela" / "Gisela Levy" (gisela.levy@ad1growth.com) / @ad1hospitality.com / @ad1growth.com → AD1 Global (recoUF9Pc03xFTuSO)
  AD1 KNOWN DEALS — when matched in dealList, emit dealReferences. When the email explicitly says a NEW engagement was scoped/signed/agreed and it's clearly distinct from anything in dealList, emit a newDealCandidates entry (companyId + dealName + evidence + confidence ONLY — never include dealValue, fees, or probability). (Trace: 2026-05-19 cleanup — Sonnet was extracting acquisition-target prices, capital-required figures, and NOI projections from deck prose and dropping them into dealValue, creating 5 phantom MR9 deals with wrong values. Strict rule: dollar figures in prose are never dealValue.):
  - "AD1 Hotel Acquisition Search" / "AD1 hotel hunt" / "AD1 hotel acquisition" / "find hotels for AD1" / "$1K a week" / "weekly retainer" — Active engagement starting May 2026, $1K/week × 3 months retainer for AD1 hotel-acquisition deal sourcing.
...
Output STRICT JSON only — no preamble, no markdown fences:
{
  ...
  "newDealCandidates": [
    {
      "companyId": "rec... (MUST match an id in the (A) list — never invent)",
      "dealName": "short descriptive engagement name (e.g. 'AD1 Hotel Acquisition Search', 'Toyosa Phase 3 Implementation'). Use a NEW name only when this is a genuinely new engagement scope distinct from the open deals in dealList below.",
      "evidence": "1-2 sentence quote/paraphrase showing the email explicitly scoped/signed/agreed a new engagement — e.g. 'Alex confirmed the $1K/week hotel-acquisition retainer starts May 1'. Do NOT emit a candidate based on acquisition prices, capital required, NOI projections, or listing prices mentioned in prose.",
      "confidence": "high | medium | low"
    }
  ],
  "newContactCandidates": [
    {
      "companyId": "rec... (MUST match an id in the (A) list)",
      "name": "person's full name as written",
      "email": "their email if mentioned (else omit)",
      "role": "their role/title if clearly stated (else omit)",
      "evidence": "1-2 sentence quote/paraphrase introducing this person at the company",
      "confidence": "high | medium | low"
    }
  ],
  "listingReferences": [
...
- newDealCandidates: emit ONLY when the email explicitly says a new engagement was SCOPED, SIGNED, AGREED, or STARTED for an existing client AND it's clearly distinct from any deal already in dealList. confidence="high" only when there's an explicit agreement signal ("we signed", "starts May 1", "$X/month retainer agreed", "scope of work approved"). NEVER emit for: acquisition target prices, capital required, listing prices, NOI projections, or any prose containing dollar figures unless those figures are OUR fee. NEVER include dealValue, monthlyRecurringFee, diagnosticFee, probability, or any dollar figure — Kevin fills those in by hand.
- newContactCandidates: emit ONLY when the email introduces a NEW person at an existing client company (companyId must match the (A) list) AND the person has identifying info (email OR phone OR a clear role). Skip casual mentions of names already established. confidence="high" only when there's an email address or clear title attached.
...
function validateTeamDiscussion(obj) {
    ...
    const out = {
        ...
        // Strict parse — NO dealValue, fees, or probability fields are read,
        // even if Sonnet returns them. Kevin fills those in manually after
        // the deal is auto-created. (Trace: 2026-05-19 MR9 cleanup.)
        newDealCandidates: Array.isArray(obj.newDealCandidates) ? obj.newDealCandidates
            .filter(c => c && typeof c.companyId === 'string' && c.companyId.startsWith('rec')
                && typeof c.dealName === 'string' && c.dealName.trim().length >= 3)
            .map(c => ({
                companyId: c.companyId,
                dealName: c.dealName.trim().slice(0, 200),
                evidence: typeof c.evidence === 'string' ? c.evidence.slice(0, 500) : '',
                confidence: ['high', 'medium', 'low'].includes(c.confidence) ? c.confidence : 'low',
            })) : [],
        newContactCandidates: Array.isArray(obj.newContactCandidates) ? obj.newContactCandidates
            .filter(c => c && typeof c.companyId === 'string' && c.companyId.startsWith('rec')
            ...
            .map(c => ({
                ...
                role: typeof c.role === 'string' ? c.role.trim().slice(0, 200) : '',
                evidence: typeof c.evidence === 'string' ? c.evidence.slice(0, 500) : '',
                confidence: ['high', 'medium', 'low'].includes(c.confidence) ? c.confidence : 'low',
            })) : [],
