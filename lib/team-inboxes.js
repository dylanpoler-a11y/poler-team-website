 ...
 *
 * Table schema (created via MCP, see plan):
...
export async function listActiveInboxes() {
    ...
    if (!res.ok) {
        ...
        if (res.status === 404 || /not.found|model/i.test(msg)) {
            throw new Error(`Team Inboxes table missing — create it in base ${baseId} with fields: Email (text, primary), Owner (single select), Refresh Token (long text), Last Polled (date+time), Active (checkbox).`);
        }
