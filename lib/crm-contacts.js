...
export async function getEmailIndex() {
    ...
    // Development Projects — LP/partner contacts indexed manually.
    // Airtable's Development Projects table has GPs in a free-text field,
    // not a related Contacts table, so we hardcode the email→projects map.
    //
    // SCHEMA: { email, name, projects: [{ projectId, projectName }, ...] }
    // The same person CAN appear on multiple projects — the cron writes
    // activity to ALL matched projects. When a new dev project starts and
    // any of these folks join, add the new {projectId, projectName} to their
    // projects array.
    //
    // Multi-project rationale (Kevin 2026-05-19): "Ed Saias, Andres, Leon are
    // constantly looking for new projects to invest in. When one arises, a
    // new Project gets created and they're added."
    const DEV_PROJECT_CONTACTS = [
        { email: 'edsaias1@gmail.com',      name: 'Eduardo (Ed) Saias',  projects: [{ projectId: 'recsTQKav0lZCrsXt', projectName: 'Belle Meade Investments LLC' }] },
        { email: 'andreshaus@gmail.com',    name: 'Andres Hausmann',     projects: [{ projectId: 'recsTQKav0lZCrsXt', projectName: 'Belle Meade Investments LLC' }] },
        { email: 'leonlevyc@gmail.com',     name: 'Leon Levy',           projects: [{ projectId: 'recsTQKav0lZCrsXt', projectName: 'Belle Meade Investments LLC' }] },
        { email: 'anidjar9770@gmail.com',   name: 'Anidjar',             projects: [{ projectId: 'recsTQKav0lZCrsXt', projectName: 'Belle Meade Investments LLC' }] },
        { email: 'mbubis@gmail.com',        name: 'Mauricio Bubis',      projects: [{ projectId: 'recsTQKav0lZCrsXt', projectName: 'Belle Meade Investments LLC' }] },
        { email: 'toledanoabi1@icloud.com', name: 'Abi Toledano',        projects: [{ projectId: 'recsTQKav0lZCrsXt', projectName: 'Belle Meade Investments LLC' }] },
    ];
    for (const dpc of DEV_PROJECT_CONTACTS) {
        const email = dpc.email.toLowerCase().trim();
        // If this person is on multiple projects, emit an array of matches.
        // The cron's writer iterates matchedRecords and writes to each.
        const matches = dpc.projects.map(p => ({
            recordType: 'development-project',
            recordId: p.projectId,
            recordName: dpc.name,
            projectId: p.projectId,
            projectName: p.projectName,
            lastTouchedAt: Date.now(),
        }));
        // Existing map allows ONE entry per email. To support multi-project,
        // we extend the map's value to be EITHER a single entry OR an array.
        // Callers (buildParticipantMatches) check Array.isArray and expand.
        if (!map.has(email)) {
            map.set(email, matches.length === 1 ? matches[0] : matches);
        }
    }
 ...
 */
// Words in client names that are too common/generic to match on their own.
// (Matching "Holdings" alone would hit "holdings co", "holdings llc", etc.)
const GENERIC_WORDS = new Set([
    'the', 'and', 'inc', 'llc', 'ltd', 'corp', 'corporation', 'company',
    'holdings', 'group', 'series', 'solution', 'solutions', 'travel',
    'team', 'hotel', 'hotels', 'real', 'estate', 'partners', 'consulting',
    'global', 'international', 'agency', 'studio', 'media', 'capital',
]);
...
export function findClientMentions(plainText, clientList) {
    ...
    const matched = new Map(); // dedupe by client id
    for (const c of clientList) {
        const nameLc = (c.name || '').toLowerCase().trim();
        // (1) Try full-name substring (length >= 4)
        if (nameLc.length >= 4 && lc.includes(nameLc)) {
            matched.set(c.id, c);
            continue;
        }
        // (2) Try each distinctive word (>= 5 chars, not generic) — catches
        // "Toyosa" matching "Intermex / Toyosa", "Giordano" matching
        // "Grupo Giordano", "DoubleTree" matching "DoubleTree by Hilton Santiago".
        const words = nameLc.split(/[\s\/\-,]+/).filter(Boolean);
        for (const w of words) {
            if (w.length < 5) continue;
            if (GENERIC_WORDS.has(w)) continue;
            // require a word-boundary on at least one side to avoid e.g.
            // "series" matching "miniseries"
        ...
        // (3) Any contact email
        if (!matched.has(c.id)) {
            if (c.emails.some(e => e && lc.includes(e.toLowerCase()))) matched.set(c.id, c);
        }
    }
...
/**
 * Detect "new client" signals in an internal team email body — phrases that
 * strongly suggest the team is discussing a (potentially new) client engagement.
 * Used to widen the team-discussion gate so we don't silently miss emails about
 * brand-new clients that aren't in the CRM yet.
 *
 * Returns true if any signal phrase is present (case-insensitive).
 */
export function hasNewClientSignals(plainText) {
    if (!plainText) return false;
    const lc = plainText.toLowerCase();
    const signals = [
        // English
        'scope of work', 'engagement letter', 'engagement with', 'new client',
        'kickoff', 'kick off', 'kick-off', 'pitching', 'pitch deck',
        'work tracker', 'proposal for', 'starting to work',
        // Spanish
        'nuevo cliente', 'propuesta para', 'carta de compromiso',
        'para la reunión con', 'para la reunion con',
        'arranque con', 'arrancar con', 'cliente nuevo',
        'alcance del trabajo', 'plan de trabajo',
    ];
