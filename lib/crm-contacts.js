...
export async function getEmailIndex() {
    ...
    // not a related Contacts table, so we hardcode the email→project map.
    // Update this list when LPs change. Added 2026-05-19.
    const DEV_PROJECT_CONTACTS = [
        // Belle Meade Investments LLC (7601 NE 9th Ave) — recsTQKav0lZCrsXt
        { email: 'edsaias1@gmail.com',         name: 'Eduardo (Ed) Saias',    projectId: 'recsTQKav0lZCrsXt', projectName: 'Belle Meade Investments LLC' },
        { email: 'andreshaus@gmail.com',       name: 'Andres Hausmann',       projectId: 'recsTQKav0lZCrsXt', projectName: 'Belle Meade Investments LLC' },
        { email: 'leonlevyc@gmail.com',        name: 'Leon Levy',             projectId: 'recsTQKav0lZCrsXt', projectName: 'Belle Meade Investments LLC' },
        { email: 'anidjar9770@gmail.com',      name: 'Anidjar',               projectId: 'recsTQKav0lZCrsXt', projectName: 'Belle Meade Investments LLC' },
        { email: 'mbubis@gmail.com',           name: 'Mauricio Bubis',        projectId: 'recsTQKav0lZCrsXt', projectName: 'Belle Meade Investments LLC' },
        { email: 'toledanoabi1@icloud.com',    name: 'Abi Toledano',          projectId: 'recsTQKav0lZCrsXt', projectName: 'Belle Meade Investments LLC' },
    ];
    for (const dpc of DEV_PROJECT_CONTACTS) {
        const email = dpc.email.toLowerCase().trim();
        const entry = {
            recordType: 'development-project',
            recordId: dpc.projectId,
            recordName: dpc.name,
            projectId: dpc.projectId,
            projectName: dpc.projectName,
            lastTouchedAt: Date.now(),
        };
        if (!map.has(email)) map.set(email, entry);
    }
