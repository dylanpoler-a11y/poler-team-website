...
import { PRECONSTRUCTIONS } from './preconstructions-data.js';
...
// ── CURATED PRECON TOWERS ────────────────────────────────────────────────────
// A 'Preconstruction' profile means the /preconstruction directory's off-plan
// TOWERS (payment plans, 2026-2031 delivery) — Kevin 2026-08-05, Yasser Lenis:
// "when I say preconstructions I mean these buildings that are getting built…
// that's what needs to appear", after MLS-only sends read as resale to him.
// Towers are returned as Bridge-shaped pseudo-listings so dedup / no-repeat /
// email / WhatsApp all work unchanged; PreconTower:true marks them for the
// renderers (own URL, "Desde $" pricing, no MLS# line, no share-property call).
// MLS New/Under Construction units still flow as BACKFILL behind the towers —
// only ~41 towers exist, so a weekly no-repeat feed exhausts them in weeks.
function towerToPseudoListing(t, siteBase) {
    const beds = Array.isArray(t.bedrooms) && t.bedrooms.length
        ? (Math.min(...t.bedrooms) === 0 ? 'Studio' : Math.min(...t.bedrooms)) +
          (t.bedrooms.length > 1 ? `-${Math.max(...t.bedrooms)}` : '')
        : '';
    return {
        ListingId: `PRECON:${t.id}`,
        ListPrice: t.priceFrom || 0,
        City: t.area || t.city || '',
        StateOrProvince: 'FL',
        UnparsedAddress: t.name,
        BedroomsTotal: beds,
        BathroomsTotalInteger: null,
        PropertySubType: 'Preconstrucción',
        Media: (t.photos || []).slice(0, 1).map(u => ({ MediaURL: u })),
        Latitude: null,
        Longitude: null,
        PreconTower: true,
        PreconUrl: `/preconstruction?building=${encodeURIComponent(t.id)}`,
        PreconDelivery: t.deliveryYear || null,
    };
}

export function searchPreconTowers(profile) {
    const cities = (profile.cities || '').split(/[,\n]/).map(s => s.trim().toLowerCase()).filter(Boolean);
    const pMax = Number(profile.priceMax) || 0;
    const bMin = Number(profile.bedsMin) || 0;
    return PRECONSTRUCTIONS
        .filter(t => {
            if (!t.priceFrom) return false; // no public pricing → can't honestly match a budget
            // City ↔ area is substring-matched BOTH ways so "Miami" reaches Brickell-adjacent
            // areas literally containing "Miami", and "Brickell" (an area, not a city) works too.
            if (cities.length) {
                const area = String(t.area || '').toLowerCase();
                const city = String(t.city || '').toLowerCase();
                const ok = cities.some(c => area.includes(c) || city.includes(c) || c.includes(area));
                if (!ok) return false;
            }
            // Entry price under the budget cap. priceMin is deliberately ignored: a tower
            // "from $250k" still has units inside a $300k+ budget (see pricingFrom tiers).
            if (pMax && t.priceFrom > pMax) return false;
            if (bMin && Array.isArray(t.bedrooms) && t.bedrooms.length
                && Math.max(...t.bedrooms) < bMin) return false;
            return true;
        })
        .sort((a, b) => (a.deliveryYear || 9999) - (b.deliveryYear || 9999) || a.priceFrom - b.priceFrom)
        .map(t => towerToPseudoListing(t));
}
...
export async function searchListingsForLead(bridgeToken, lead, options = {}) {
    ...
    for (const rawProfile of profiles) {
        ...
        // Precon profiles lead with the curated /preconstruction TOWERS; the MLS
        // New/Under Construction units just fetched become backfill behind them.
        if ((profile.features || []).includes('Preconstruction')) {
            const towers = searchPreconTowers(profile);
            debug.towers = towers.length;
            listings.unshift(...towers);
        }
    ...
    // Rank order (stable, survives the shuffle): fresh towers → fresh MLS →
    // already-sent towers → already-sent MLS. The shuffle above still randomizes
    // WITHIN each rank, so MLS backfill stays varied week to week while curated
    // towers always take the front slots for precon profiles.
    if (excl.size > 0 || unique.some(l => l.PreconTower)) {
        const rank = l => (excl.has(l.ListingId) ? 2 : 0) + (l.PreconTower ? 0 : 1);
        unique = unique.map((l, i) => [rank(l), i, l])
