...
- **Backfill:** `cloud-sender/backfill_leadgen.py --go` under `railway run` — see counts in the chat summary / `state/backfill_leadgen_log.jsonl`. Sources: `autoresearch/data/reply_events.jsonl` (458 events, no bodies) + ledgers; text pulled from 30 mailboxes (IMAP `X-GM-RAW` + Gmail API). Excluded on purpose: 33 real-estate-campaign replies (lauderdale_hotel/expired_3m/waterfront → Real Estate `buyer:<MLS>`), 39 `agent_warmup`, 133 auto. ~14 Instantly-era repliers had no recoverable text.
