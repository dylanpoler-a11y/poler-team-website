...
- **OUTREACHES** (nav label since 2026-09-21; was "Lead Generation", tables/API/code keep the `LeadGen`/`leadgen` names): every outreach contact we EMAILED — `Prospect` = no answer yet (cadence leads auto-added by `email-cadences/add-cadence.mjs`), plus every contact who REPLIED (any sentiment, auto-replies/OOO/bounces dropped) — agency email (Railway cloud-sender; earlier rows predating Sept 2026 backfilled under the same channel `Email`), Facebook, LinkedIn, Keystone/Abrams investor outreach. Tables `LeadGen Leads` / `LeadGen Tasks` / `LeadGen Activity`; ingest endpoint `POST /api/agent/leadgen-reply` (idempotent on `messageId`). **Full detail → [`docs/crm-internals.md`](docs/crm-internals.md).**
