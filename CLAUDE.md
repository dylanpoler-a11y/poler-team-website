...
- **LEAD GENERATION** (2026-09-10): every outreach contact who REPLIED (any sentiment, auto-replies/OOO/bounces dropped) — agency email (Railway cloud-sender; earlier rows predating Sept 2026 backfilled under the same channel `Email`), Facebook, LinkedIn, Keystone/Abrams investor outreach. Tables `LeadGen Leads` / `LeadGen Tasks` / `LeadGen Activity`; ingest endpoint `POST /api/agent/leadgen-reply` (idempotent on `messageId`). **Full detail → [`docs/crm-internals.md`](docs/crm-internals.md).**
