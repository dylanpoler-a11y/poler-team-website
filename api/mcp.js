...
const TOOLS = [
    ...
    },
    {
        ...
        description: 'Record an inbound reply to outreach. Drops auto-replies/OOO/bounces, classifies sentiment, keeps a running conversation summary, dedups on sourceLeadId then email. Consulting-client campaigns (Plaza San Miguel/Atrio, Mara-CSC) are routed to the Consulting module automatically.',
        inputSchema: {
            type: 'object',
            properties: {
                name:           { type: 'string' },
                ...
                replyAt:        { type: 'string', description: 'ISO timestamp of the reply' },
                sourceLeadId:   { type: 'string', description: 'Stable id from the sending system, for dedup' },
                subject:        { type: 'string' },
                ourLastMessage: { type: 'string' },
                sentiment:      { type: 'string', description: 'Override the classifier' },
                status:         { type: 'string', description: 'Pipeline status to set (New|Contacted|Meeting Booked|Won|Lost); never downgrades Won/Meeting Booked' },
                messageId:      { type: 'string', description: 'Sender-side message id — a second push with the same id is a no-op (skipped: duplicate)' },
                force:          { type: 'boolean', description: 'Ingest even if the text looks like an auto-reply/opt-out' },
            },
            required: ['replyText'],
        },
        endpoint: { method: 'POST', path: '/api/agent/leadgen-reply' },
    },
    {
        name: 'leadgen_update_lead',
        description: 'Update a Lead Generation lead: pipeline status, sentiment, summary, notes, owner, contact fields.',
        inputSchema: {
            type: 'object',
            properties: {
                id:        { type: 'string' },
        ...
        endpoint: { method: 'PATCH', path: '/api/update-leadgen-lead' },
    },
    {
        name: 'leadgen_log_activity',
        description: 'Append an activity row (note, call, meeting, email sent) to a Lead Generation lead.',
        inputSchema: {
            type: 'object',
            properties: {
                leadId:  { type: 'string' },
                title:   { type: 'string' },
                type:    { type: 'string', description: 'Note | Call | Meeting | Email Sent | Reply' },
                details: { type: 'string', description: 'Full text (for emails: the whole body)' },
                subject: { type: 'string', description: 'Email subject (Email Sent / Reply rows)' },
                mailbox: { type: 'string', description: 'Mailbox the email was sent from / received at' },
                messageId: { type: 'string', description: 'RFC Message-ID — makes the call idempotent (upserts an existing row)' },
                at:      { type: 'string', description: 'ISO timestamp; defaults to now' },
                agent:   { type: 'string', description: 'Kevin | Responder Bot | Dylan | Rosa | Flash Coach' },
                attachments: { type: 'array', description: 'Files to attach (≤4 MB each, one big file per call)',
                    items: { type: 'object', properties: { filename: { type: 'string' }, contentType: { type: 'string' }, base64: { type: 'string' } }, required: ['filename', 'base64'] } },
            },
            required: ['leadId', 'title'],
        },
        endpoint: { method: 'POST', path: '/api/log-leadgen-activity' },
    },
    {
        name: 'leadgen_get_activity',
        description: 'Timeline of a Lead Generation lead (replies, notes, stage changes).',
        inputSchema: {
            type: 'object',
            properties: { leadId: { type: 'string' }, limit: { type: 'number' } },
        },
        endpoint: { method: 'GET', path: '/api/get-leadgen-activity' },
    },
    {
